...
public class Picture {
	...
	static BufferedImage masque(BufferedImage image,BufferedImage masque){
	 ...
	 // convolution par une matrice carré de la taille du filtre
	 // sur les bors on réalis eun mirroir
		 ...
		 for(int x=0;x<image.getWidth();x++){
			 for(int y=0;y<image.getHeight();y++){
				 ...
				 for(int xFiltre=0;xFiltre<tailleFiltre;xFiltre++){
					 ...
					 for(int yFiltre=0;yFiltre<tailleFiltre;yFiltre++){
						 int Xpoint = Math.abs(x+xFiltre-demiTailleFiltre);
						 int Ypoint = Math.abs(y+yFiltre-demiTailleFiltre);
						 if (Xpoint>=image.getWidth()) Xpoint = image.getWidth()-1;
						 if (Ypoint>=image.getHeight()) Ypoint = image.getHeight()-1;
						 //System.out.println(image.getWidth()+" "+image.getHeight()+"|"+Xpoint+" "+Ypoint);
						 Color c = new Color(image.getRGB(Xpoint, Ypoint));
	 ...
	 public static BufferedImage rotate90(BufferedImage image){
		 BufferedImage imageReturn=create(image.getHeight(), image.getWidth());
		 
		 for(int x=0;x<image.getWidth();x++){
			 for(int y=0;y<image.getHeight();y++){
				 Color c1 = new Color(image.getRGB(x, y));
				 imageReturn.setRGB(y, x, c1.getRGB());
			 }
		 }
		 return imageReturn;
	 }

	 public static ArrayList projectionX(BufferedImage image){
		return null;
